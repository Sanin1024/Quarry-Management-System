...
function initTheme() {
    ...
    document.querySelectorAll('button').forEach(btn => {
        if (btn.querySelector('.material-symbols-outlined') && btn.querySelector('.material-symbols-outlined').textContent.trim() === 'dark_mode') {
...
function bindNavigation() {
    document.querySelectorAll('a').forEach(link => {
        ...
        else if (text.includes('quarry 1')) link.href = 'quarry1.html';
        else if (text.includes('quarry 2')) link.href = 'quarry2.html';
        else if (text.includes('compare')) link.href = 'comparison.html';
        else if (text.includes('import data') || text.includes('import excel')) link.href = 'import.html';
        else if (text.includes('reports')) link.href = 'reports.html';
    });
    document.querySelectorAll('button').forEach(btn => {
        const text = btn.textContent.trim().toLowerCase();
        if (text.includes('new entry') || text.includes('add working period')) {
            btn.addEventListener('click', () => {
                const isQ2 = window.location.pathname.includes('quarry2');
                window.location.href = isQ2 ? 'add_entry.html?quarry=q2' : 'add_entry.html?quarry=q1';
            });
        } else if (text.includes('import excel')) {
            btn.addEventListener('click', () => {window.location.href = 'import.html';});
        }
    });
}
...
function showToast(message, type='success') {
    ...
    if (type === 'success') {toast.className += ' bg-success'; toast.innerHTML = `<span class="material-symbols-outlined">check_circle</span> ${message}`;}
    else if (type === 'error') {toast.className += ' bg-error'; toast.innerHTML = `<span class="material-symbols-outlined">error</span> ${message}`;}
    else {toast.className += ' bg-secondary'; toast.innerHTML = `<span class="material-symbols-outlined">info</span> ${message}`;}
    document.body.appendChild(toast);
    setTimeout(()=>{toast.classList.remove('translate-y-10','opacity-0');},10);
    setTimeout(()=>{toast.classList.add('translate-y-10','opacity-0'); setTimeout(()=>{toast.remove();},300);},3000);
}

document.addEventListener('DOMContentLoaded',()=>{initializeDatabase();initTheme();bindNavigation();});
